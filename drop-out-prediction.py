...
def train_model():
    ...
    # ── XGBoost classifier (multiclass softmax)
    xgb = XGBClassifier(
        n_estimators=300,
        max_depth=6,
        learning_rate=0.1,
    ...
    return xgb, acc, cm, fi, report, len(df), df
...
# LOAD MODEL
# ─────────────────────────────────────────────
xgb_model, accuracy, conf_matrix, feat_importance, class_report, n_samples, df_raw = train_model()
...
if page == "🏠 Overview":
    ...
    with col3:
        ...
        ax2.legend(fontsize=7, labelcolor='white', facecolor='#111827',
...
# ─────────────────────────────────────────────
    ...
    if submitted:
        ...
        pred_label = TARGET_CLASSES[int(pred)]
    ...
    if uploaded:
        try:
            ...
            if missing:
                ...
                preds  = xgb_model.predict(X_np_batch).astype(int)
                probas = xgb_model.predict_proba(X_np_batch)
                ...
                <div class="metric-row">
                    ...
                    <div class="metric-card" style="--accent-line:linear-gradient(90deg,#68d391,#9ae6b4);--accent-grad:linear-gradient(90deg,#68d391,#9ae6b4);">
                        ...
                        <div class="metric-label">PREDICTED LOW RISK ({gr_pct:.0%})</div>
                    </div>
                ...
                st.download_button("⬇️ Download Results as Excel", buf.getvalue(),
                                   file_name="predictions_xgboost.xlsx",
...
# ─────────────────────────────────────────────
    ...
    with c2:
        ...
        <div class="info-card">
            ...
            <p>Python · Streamlit · XGBoost · scikit-learn · pandas · NumPy</p>
        </div>
        </div>
        <div class="info-card">
            <h4>⚠️ Ethical Note</h4>
            <p>Predictions are decision-support tools, not deterministic labels. 
            Human judgment should always be applied when interpreting results and designing interventions.</p>
        </div>
        """, unsafe_allow_html=True)
